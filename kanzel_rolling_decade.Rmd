...
rm(list=ls())
#
kanzel <- fread("./../db/combined_isn.csv")
...
```
```{r echo=FALSE}
ggplot(data=df,aes(g)) +geom_histogram(binwidth=2) + 
...
```
```{r echo=FALSE}
ggplot(data=df,aes(s)) +geom_histogram(binwidth=20) + facet_wrap(~Year,nrow =4,ncol=4,scale="free_y") + 
